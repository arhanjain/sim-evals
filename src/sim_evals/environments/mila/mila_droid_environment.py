...
from sim_evals.environments.droid.env_cfg import (
    DroidEnvCfg,
    EventCfg as DroidEventCfg,
    SceneCfg as DroidSceneCfg,
)
from sim_evals.environments.droid.mdp import reset_initial_conditions
from sim_evals.environments.droid.scenes import get_data_path

from .rewards import milestone_reward, reset_milestones
from .tasks import CameraCfg, MilaTask, get_mila_task


MILA_ASSET_PATH = get_data_path() / "mila"
...
class MilaDroidEnvCfg(DroidEnvCfg):
    ...
    def set_task(self, task_id: str) -> "MilaDroidEnvCfg":
        """Compose a concrete MILA task onto this config; returns ``self``.

        Mirrors :meth:`DroidEnvCfg.set_scene` for the MILA parity tasks: attaches
        the task's shared scene, milestone rewards, deterministic initial
        conditions, instruction, and the external camera the policy consumes.
        """
        task = get_mila_task(task_id)
        ...
        self.language_instruction = task.language_instruction
        # MILA repositions ``external_cam``; the policy view is ``external_cam_2``.
        self.policy_external_camera = "external_cam_2"
        return self
